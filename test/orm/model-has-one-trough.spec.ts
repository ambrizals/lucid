...
import type { HasOneThrough } from '@ioc:Adonis/Lucid/Orm'

// import { scope } from '../../src/Helpers/scope'
import { hasOneThrough, column } from '../../src/Orm/Decorators'
...
test.group('Model | HasOneThrough | Options', (group) => {
  group.before(async () => {
  ...
  test('raise error when localKey is missing', (assert) => {
    ...
    try {
      class User extends BaseModel {
        @hasOneThrough([() => Provence, () => PostalCode])
        public provence: HasOneThrough<typeof Provence>
      }
      User.boot()
      ...
      class Provence extends BaseModel {}
      Provence.boot()

      User.$getRelation('provence')!.boot()
    } catch ({ message }) {
      assert.equal(
        message,
        'E_MISSING_MODEL_ATTRIBUTE: "User.provence" expects "id" to exist on "User" model, but is missing'
      )
    }
  })
  ...
  test('raise error when foreignKey is missing', (assert) => {
    ...
    try {
      ...
      assert.equal(
        message,
        'E_MISSING_MODEL_ATTRIBUTE: "User.provence" expects "userId" to exist on "PostalCode" model, but is missing'
      )
    }
  })
  ...
  test('raise error when through local key is missing', (assert) => {
    ...
    try {
      ...
      assert.equal(
        message,
        'E_MISSING_MODEL_ATTRIBUTE: "User.provence" expects "id" to exist on "PostalCode" model, but is missing'
      )
    }
  })

  test('raise error when through primary key is missing', (assert) => {
    assert.plan(1)
